...
<body>
    ...
    <?php
        ...
        // -----------------------------------------FOR TABLE-----------------------------------------
        // // // call procedure 2
        if (!$conn->multi_query("INSERT INTO `WISHLIST`(`Username`, `ProdID`) VALUES ('$username', '$id')")) {
            echo "CALL failed: (" . $mysqli->errno . ") " . $mysqli->error;
            $result = "$Type has failed to be added to Wishlist.";
        }
        else{
            $result = "$Type has been added to Wishlist.";
        }
    ?>
    <div class="wrapper"> 
            <!--Header template!-->
